...
async function refreshBankAccounts() {{
  ...
  // Llenar select de cuentas bancarias
...
async function loadProductsForInvoice() {{
  ...
  if (!data.products || !data.products.length) {{
    $('#invoiceProductSelection').innerHTML = '<p style="color: var(--muted);">No hay productos disponibles en el inventario.</p>';
    return;
  }}
  
  $('#invoiceProductSelection').innerHTML = data.products.map(p => `
...
function renderInvoiceAssistantHistory() {{
  ...
  container.innerHTML = invoiceAssistantHistory.map(msg => `
    <div class="conversation ${{msg.role === 'user' ? 'user' : ''}}">
      <strong>${{msg.role === 'user' ? 'Tú' : 'Asistente'}}${msg.source ? ' · ' + msg.source : ''}:</strong>
