# Copy/Paste Package for GitHub: Proyecto Sin Pausas

This document contains everything needed to manually recreate the Sabrina AI Lab MVP in GitHub.
...
Create this exact structure in the GitHub repository:
...
2. Create or edit each file listed below.

3. Copy the content exactly from each section.

4. Commit the files to the `main` branch.

5. After committing, the app can be run on the VM with:
   ```bash
...
Only upload the files included in this document unless you intentionally want to include more project history.

---

# File: `README.md`

Description: Root repository README
...
# File: `proyectos/sabrina_ai_lab/README.md`

Description: MVP documentation
...
# File: `proyectos/sabrina_ai_lab/app.py`

Description: Functional frontend + backend app
...
# Optional command-line upload method

If you are on the VM and GitHub authentication is configured, you can upload these files with:
...
git add README.md .gitignore proyectos/sabrina_ai_lab/README.md proyectos/sabrina_ai_lab/app.py
git commit -m "Add Sabrina AI Lab MVP" || true
git branch -M main
git push -u origin main
```

Do not use `git add .` from `/home/Sabrina` unless you have reviewed all untracked files.
...
Success means the hash from `git ls-remote --heads origin main` matches `git rev-parse HEAD`.
